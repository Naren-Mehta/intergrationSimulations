...
class TrafficUtilService {
    ...
    public Boolean performLogic(Traffic traffic) {
        ...
        List<IpAddressDocuments> ipAddressDocumentsList = traffic.IpAddressDocuments?.findAllByTraffic(traffic)
        ...
        for (int intt = 0; intt < aaa.length; intt++) {
            ...
            for (int j = 0; j < test.length; j++) {
                for (int jj = j; jj < (j + batchSize); jj++) {
                    ...
                    if (is1) {
                        ...
                        if (is2) {
                            ...
                            List<DnsblsUrls> dnsblsUrlses = traffic.DnsblsUrls?.findAllByTraffic(traffic)
                            if (dnsblsUrlses) {
                ...
                AppUtilTraffic?.save(staticsResults)
    ...
    def storeIpAddress(CommonsMultipartFile uploadedFile, Traffic traffic) {
        if (uploadedFile?.bytes) {
            ...
            AppUtilTraffic?.save(ipAddressDocuments)
        }
    }
    ...
    def storeSystemLogs(CommonsMultipartFile uploadedFile, Traffic traffic) {
        if (uploadedFile?.bytes) {
            ...
            AppUtilTraffic?.save(systemLogsDocuments)
        }
    }
